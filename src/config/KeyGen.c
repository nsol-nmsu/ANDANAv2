...
int main() {
    ...
    char   pkFileName[KEY_LENGTH/8];  // Message to encrypt
    char   skFileName[KEY_LENGTH/8];  // Message to encrypt
    ...
    fgets(pkFileName, KEY_LENGTH-1, stdin);
    pkFileName[strlen(pkFileName)-1] = '\0';
    pkFile = fopen(pkFileName, "w");

    printf("Private key file: ");
    fgets(skFileName, KEY_LENGTH-1, stdin);
    skFileName[strlen(skFileName)-1] = '\0';
    skFile = fopen(skFileName, "w");
    ...
    BIO *pri = BIO_new_fp(skFile, BIO_CLOSE);
    BIO *pub = BIO_new_fp(pkFile, BIO_CLOSE);
